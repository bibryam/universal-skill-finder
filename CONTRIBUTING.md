...
Preserve the plugin response contract: every configured source is accounted for; search requests up to 20 native candidates per source, freezes the complete deduplicated ranking, retains up to 100 results, and shows 25 compact two-line entries by default. A search-result name may link only to a connector-reviewed registry route or normalized GitHub identity, and that link is discovery evidence rather than a liveness or installation claim. All other report-derived links require eligible proof for the exact browsing destination. The fixed, application-owned project footer is the only exception. `Next page` and `Show all` must use the frozen pool without network access or reranking. `Inspect #N` may update proof evidence but must not change order or numbering. Raw `SKILL.md` target proof cannot authorize a directory, repository, or registry link; commands additionally require exact target proof. Cached results must not imply a live request. Generate commands locally from validated metadata, never copy executable hints from registry responses, and never execute during search.
...
When releasing, rename the `Unreleased` changelog heading to `VERSION - YYYY-MM-DD` using the actual release date, then add a fresh `Unreleased` section for subsequent work. For final promotion, review the complete tracked-file inventory and obtain approval to commit/tag the release. Build from the clean commit tagged with its matching `vVERSION`:
