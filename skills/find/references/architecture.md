...
Completed online reports end with a single-line star reminder, without ASCII art. Its repository link requires eligible proof; otherwise it stays plain text with a verification caveat. The footer adds no network work and is absent from previews, offline output, help, source management and all-source failures.
