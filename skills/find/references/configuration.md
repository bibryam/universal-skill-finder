...
Public individual-skill rows retain structurally validated GitHub repository identity and `SKILL.md` directory, including `.agents` and `.claude` roots. Other repository providers are currently unsupported and reported as omitted. A score's version is not assumed to be a Git ref. Discovery may link the name to Tessl's code-owned `/registry/skills/github/OWNER/REPO/SKILL` route before liveness is known. **Inspect #N** checks that route and, for an unpinned result, may resolve GitHub's authoritative default branch once; it shows the skill tree and repository root only when each displayed URL passes its own check, and never guesses either destination from raw file proof. A failed or stale detail page remains unverified. Skill-bearing Tessl packages are labelled bundles with a discovery listing link and no inferred individual skill or install command. Package and individual-skill identities remain distinct.
...
A plain query searches every enabled source concurrently, requests up to 20 native candidates per source, retains up to 100 results from the deduplicated ranking, and shows 25 compact discovery entries on the first page. `--count N` changes the overall cap, `--page-size N` changes the page size, and `--per-source-limit N` controls candidate depth. Legacy `--max-results` and `--limit` remain aliases. An explicit `--report-json PATH` preserves the frozen pool and numbering for `page`, `inspect`, `explain`, and `details`; direct CLI searches create no background report store. Search links are structurally reviewed navigation routes, not live destination or installation proof. Source filters are opt-in, never required in the user's capability request.
