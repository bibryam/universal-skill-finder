...
Public individual-skill rows retain validated GitHub repository identity and `SKILL.md` directory, including `.agents` and `.claude` roots. Other repository providers are currently unsupported and reported as omitted. A score's version is not assumed to be a Git ref; missing revisions produce repository inspection links. Skill-bearing Tessl packages are labelled bundles with a verified package listing link and no inferred individual skill or install command. Package and individual-skill identities remain distinct.
